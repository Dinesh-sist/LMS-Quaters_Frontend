...
import Sidebar from "./EmployeeUI/SideNav";
...
export default function ApplyForQuarters() {
  ...
  const setQ = (key,   val) => setForm((f) => ({ ...f, quarters: { ...f.quarters, [key]: val } }));
  ...
  return (
    <div className="font-['Segoe_UI',system-ui,sans-serif] h-screen flex flex-col overflow-hidden bg-[#f4f6fa]">

      {/* ── Header ── */}
      <header className="bg-[#1a2e5a] flex items-center justify-between px-7 h-16 shrink-0 shadow-[0_2px_8px_rgba(0,0,0,0.18)] z-20">
        <div className="flex items-center gap-3.5">
          <div>
            <div className="text-white font-extrabold text-[15px]">Paradip Port Authority</div>
            <div className="text-white/45 text-[9.5px] uppercase tracking-[0.12em]">Land Data Management System</div>
          </div>
        </div>
        ...
        <nav className="flex items-center gap-7">
          {[
            ...
            <Link key={to} to={to}
              className="text-white/75 no-underline text-[13px] font-medium hover:text-white transition-colors">
              {label}
            </Link>
          ))}

          <button className="bg-transparent border-0 text-white/75 text-[13px] font-medium cursor-pointer flex items-center gap-1 hover:text-white transition-colors">
            Apply Online
            <svg width="12" height="12" fill="currentColor" viewBox="0 0 20 20">
              <path d="M5.23 7.21a.75.75 0 011.06.02L10 11.085l3.71-3.855a.75.75 0 111.08 1.04l-4.25 4.42a.75.75 0 01-1.08 0L5.21 8.27a.75.75 0 01.02-1.06z" />
            </svg>
          </button>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <main className="flex-1 overflow-y-auto px-9 py-7 bg-[#f4f6fa]">
          ...
          <div className="mb-[22px]">
            <h1 className="text-xl font-extrabold text-slate-800 tracking-[-0.02em] mb-1.5">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid items-start gap-5 mb-5" style={{ gridTemplateColumns: "200px 1fr" }}>
                ...
                <div className="flex flex-col gap-2">
                  {[
                    ...
                    <label key={opt.value}
                      className={`flex items-center gap-2 cursor-pointer text-[13.5px] text-slate-800 ${form.appliedFor === opt.value ? "font-semibold" : "font-normal"}`}>
                      <input
                        type="radio"
                        name="appliedFor"
                        value={opt.value}
              ...
              <div className="grid items-center gap-5 mb-5" style={{ gridTemplateColumns: "200px 1fr" }}>
                ...
                >
                  ...
                  {HODS.map((h) => <option key={h} value={h}>{h}</option>)}
                </select>
              </div>
